...
def sample_importance_sampling(shape, beta_min, beta_max, T,t_epsilon=1e-3):
...
def get_normalizing_constant(shape,beta_min,beta_max,T=1.0):
    if isinstance(T, float) or isinstance(T, int):
        T = torch.Tensor([T]).float()
    u = torch.rand(*shape).to(T)
    vpsde = VariancePreservingTruncatedSampling(beta_min=beta_min, beta_max=beta_max, t_epsilon=0.001)
